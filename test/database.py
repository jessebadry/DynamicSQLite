...
def compile_basic_assign_params(cols, vals, delim=','):
...
def update(table, r_cols, r_vals, where_condition='', where_val='', extra_sql=''):
    if len(r_cols) != len(r_vals):
        ...
        params = compile_basic_assign_params(r_cols, r_vals)
...
def get_sql_vals(record):
    ...
    if warning_bool:
        warning(
            ...
            'DSQLite, Spaces will be replaced with _, making model keys without spaces. ')
...
def compile_insert_sql(insert_method, table, model_record):
    ...
    return ('{} into {} {} values({}) '.format(insert_method, table, val_names, params))
...
def where(find_dict, table):
    sql = 'Select * from ' + table + ' Where ' + compile_basic_assign_params(list(find_dict.keys()), list(find_dict.values()),
                                                                      delim=' AND ')
